...
int ucs2utf8(unsigned short ucs2, char* utf8)
{
    ...
    {
        utf8[l++] = (char)( 0xc0 | ( ucs2 >> 6   ) );
        utf8[l++] = (char)( 0x80 | ( ucs2 & 0x3f ) );
    }
    else
    {
        utf8[l++] = (char)( 0xe0 | ( ucs2 >> 12 ) );
        utf8[l++] = (char)( 0x80 | ( ucs2 >> 6 & 0x3f ) );
        utf8[l++] = (char)( 0x80 | ( ucs2 & 0x3f ) );
    }
    utf8[l] = 0;
    return l;
...
int utf8ucs2(const char* utf8, unsigned short* ucs2)
{
    ...
    if (byte < 0xC0)
    {
        /* Handles properly formed UTF-8 characters between 0x01 and 0x7F. Also treats \0 and
        naked trail bytes 0x80 to 0xBF as valid characters representing themselves. */
        if (ucs2)
        {
            *ucs2 = byte;
        }
    ...
    else if (byte < 0xF0)
    {
        if (((utf8[1] & 0xC0) == 0x80) && ((utf8[2] & 0xC0) == 0x80))
        {
            ...
            if (ucs2)
            {
                *ucs2 =  ( (byte & 0x0F) << 12 )  | ( (utf8[1] & 0x3F) << 6 ) | (utf8[2] & 0x3F);
            }
            return 3;
        }
...
{
    int ucs_len = __wcslen__(ucs2s);
    int utf_len = 0;
    int i = 0;
    for ( i = 0; i < ucs_len; i++ )
    {
        utf_len += ucs2utf8(ucs2s[i], utf8s + utf_len);
    }
    return utf_len;
}
...
{
    int ucs_len = 0;
    while ( *utf8s && ucs_len < len )
    {
        utf8s   += utf8ucs2(utf8s, ucs2s);
        ucs2s++;
        ucs_len += 1;
    }
	*ucs2s = 0;
...
int IsGBKCode( unsigned short code )
{
    int  m, h, l, found;
...
int IsUniCode( unsigned short code )
{
    int  m, h, l, found;
...
unsigned short Unicode2GB(unsigned short unicode)
{
    int found = IsUniCode( unicode );
    if( found < 0 )
    {
        return 0;
    }
    ...
    {
        return UC2GBK[found].GBKCode;
    }
}

unsigned short GB2Unicode(unsigned short gbcode)
{
    int found = IsGBKCode( gbcode );
    if( found < 0 )
    {
        return 0;
    }
...
int wchar2gbk(unsigned short* wcstr, int len, char *str, int size)
{
    ...
    for ( i = 0, j = 0; i < len; i++)
    {
        ...
        else
        {
            ...
            gbk = Unicode2GB(wcstr[i]);
            if (gbk > 0)
            {
                str[j++] = ((unsigned short)(gbk) & 0xFF00 ) >> 8;
                str[j++] = ((unsigned short)(gbk) & 0x00FF ) >> 0;
            }
        }
    }
    str[j] = 0;
...
int gbk2wchar(const char *str, int len, unsigned short* wcstr, int size)
{
    ...
    for ( i = 0, j = 0; i < len; i++)
    {
        ...
        else
        {
            ...
            gcode = ((unsigned char)str[i] << 8) | ((unsigned char)str[i+1]);
            ++i;

            ucode = GB2Unicode(gcode);
            if (ucode > 0)
            {
                wcstr[j++] = ucode;
            }
