...
#define ivGrmResFile (3)    /* File deployed as Grm Resource data, Persistent while device shutdown */
#define ivCommonFile (4)	/* File deployed as Common data */

#define ivGrmDict		(1)		/*Begin a Dict as Grm Dict*/
#define ivHotWordDict	(2)		/*Begin a Dict as HotWord Dict*/
...
typedef ivStatus (ivProc PCBCopyFile)(
									 ivPointer	pUser,
									 ivCStrW  pFileName      /* Name of file to copy */
									);
...
typedef struct tagUserOS
{
	...
	PCBCopyFile     lpfnCopyFile;   /* Copy file from grammar space to recognize space */

	/* Message handler */
	PCBMsgProc		lpfnMsgProc;	/* Message process Interface */

	ivByte          nSampleRateCheck;
...
/* GrmObject Create & Destroy */
#define ivMsg_GRMCreate					(0x202)
/*----------------------------------------------------------------------------------
Notify application that the GrmObject was Created.
This message is sent when the GrmObject be created, Here the application can 
call the other initializing function, such as set GrmObject's parameter, And
Allocate and initialize the data binded with the GrmObject
Parameter: NA
Return:
ivErr_OK	: All OK.
Other		: There's some error.
==================================================================================*/
#define ivMsg_GRMDestroy                (0x203)
/*----------------------------------------------------------------------------------
Notify application that the GrmObject will be Destroyed.
This message is sent when the GrmObject will be Destroyed, Here the application
can do some anti-initializing work, such as anti-initialize and free the 
data binded with the GrmObject.
When you Destroy the GrmObject by call function EsrDestroy, the GrmObject may
NOT be really Destroyed immediately, so that the anti-initializing work of
application binded with the GrmObject can only be process here.
Parameter: NA
Return: NA
==================================================================================*/
#define ivMsg_ServiceBegin                (0x204)
/*----------------------------------------------------------------------------------
Notify application that the Service has been beginned.
Parameter: NA
Return: NA
==================================================================================*/
/* Error information */
