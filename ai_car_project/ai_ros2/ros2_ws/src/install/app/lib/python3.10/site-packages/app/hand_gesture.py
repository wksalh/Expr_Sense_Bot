...
class HandGestureControlNode(Node):
    ...
    def move_action(self, *args):
        ...
        if self.machine_type == 'MentorPi_Acker':
            self.publish_servo_state([3], [1500])  # 停止
            motor1 = MotorState()
            motor1.id = 2
            motor1.rps = 0.0
            motor2 = MotorState()
        ...
        if len(msg.points) >= 5:
            ...
            if 0 <= angle < 30:
                if sum(left_and_right) > 0:
                    if self.machine_type == 'MentorPi_Acker':
                        twist.angular.z = 1.0
                        self.publish_servo_state([3], [1179])  # 右转
                        threading.Timer(1.0, self.publish_servo_state, [[3], [1500]]).start()  # 1秒后停止
                    elif self.machine_type == 'MentorPi_Mecanum':
                        twist.linear.y = float(self.linear_speed)
                else:
                    if self.machine_type == 'MentorPi_Acker':
                        twist.angular.z = -1.0
                        self.publish_servo_state([3], [1821])  # 左转
                        threading.Timer(1.0, self.publish_servo_state, [[3], [1500]]).start()  # 1秒后停止
                    elif self.machine_type == 'MentorPi_Mecanum':
                        twist.linear.y = float(-self.linear_speed)
            elif 60 < angle <= 90:
