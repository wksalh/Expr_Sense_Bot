...
class Board:
    ...
    def set_RGB(self,id,data):
        if id in [1 , 2]:
            send = struct.pack("<BBBB" , id , data[0] , data[1] , data[2])
            self.buf_write(PacketFunction.PACKET_FUNC_RGB , send)
        elif id == 0:
            send = struct.pack("<BBBBBBB" , 0 , data[0] , data[1] , data[2], data[3] , data[4] , data[5])
            self.buf_write(PacketFunction.PACKET_FUNC_RGB , send)
...
def pwm_servo_test(board):
    servo_id = 1
    board.pwm_servo_set_position(0.5, [[servo_id, 2500]])
...
if __name__ == "__main__":
    ...
    board.set_motor_type(2)
    time.sleep(1)
    board.set_RGB(0 , [0,250,0,0,250,0])
    time.sleep(1)
    board.set_RGB(1,[0,0,0])
    board.set_RGB(2,[0,0,0])
