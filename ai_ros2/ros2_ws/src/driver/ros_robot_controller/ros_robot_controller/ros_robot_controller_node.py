...
class RosRobotController(Node):
    ...
    def __init__(self, name):
        rclpy.init()
        super().__init__(name)
        self.board = Board(device="/dev/ttyHS2")
    ...
    def load_servo_offsets(self):
        ...
        import os
        # 使用相对路径查找配置文件
        # 从当前工作目录 /mnt/ros2_ws 到 /mnt/software/Servo_upper_computer/
        config_path = os.path.join('..', 'software', 'Servo_upper_computer', 'servo_config.yaml')
        # 添加调试信息
        self.get_logger().info(f"当前工作目录: {os.getcwd()}")
        self.get_logger().info(f"配置文件路径: {os.path.abspath(config_path)}")
        self.get_logger().info(f"配置文件是否存在: {os.path.exists(config_path)}")
        try:
