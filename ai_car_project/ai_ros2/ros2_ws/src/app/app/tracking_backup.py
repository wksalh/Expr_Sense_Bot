...
class ObjectTracker:
    def __init__(self, color, node, set_color, set_status=False):
        self.node = node
        self.y_stop = 120
        self.pro_size = (480, 360)  # 优化图像处理分辨率，从(320, 240)提高到(480, 360)，平衡精度和速度
        ...
        # 云台PID参数 - 进一步降低响应速度，防止过摇
        self.servo_x_pid = pid.PID(P=0.15, I=0.02, D=0.05)  # 大幅降低P和I参数，增加D参数防止过摇
        self.servo_y_pid = pid.PID(P=0.15, I=0.02, D=0.05)  # 大幅降低P和I参数，增加D参数防止过摇
        ...
        self.pan_tilt_x_threshold = 25  # 进一步增大云台X轴死区，防止微小抖动
        self.pan_tilt_y_threshold = 25  # 进一步增大云台Y轴死区，防止微小抖动
        
        # 舵机位置平滑处理参数
        self.last_servo_x = 1500
        self.last_servo_y = 1500
        self.servo_smooth_alpha = 0.3  # 舵机位置平滑系数，防止过摇
    ...
    def update_pid(self, x, y, img_w, img_h):
        ...
        # 大幅降低所有误差范围的PID参数，防止过摇
        if error_x > 80:
            self.servo_x_pid.Kp = 0.25
            self.servo_x_pid.Ki = 0.03
            self.servo_x_pid.Kd = 0.08
        elif error_x > 40:
            self.servo_x_pid.Kp = 0.18
            self.servo_x_pid.Ki = 0.02
            self.servo_x_pid.Kd = 0.06
        else:
            self.servo_x_pid.Kp = 0.12
            self.servo_x_pid.Ki = 0.01
            self.servo_x_pid.Kd = 0.04
            
        if error_y > 80:
            self.servo_y_pid.Kp = 0.25
            self.servo_y_pid.Ki = 0.03
            self.servo_y_pid.Kd = 0.08
        elif error_y > 40:
            self.servo_y_pid.Kp = 0.18
            self.servo_y_pid.Ki = 0.02
            self.servo_y_pid.Kd = 0.06
        else:
            self.servo_y_pid.Kp = 0.12
            self.servo_y_pid.Ki = 0.01
            self.servo_y_pid.Kd = 0.04
        ...
        # 限制舵机输出变化幅度，防止过摇
        servo_x_output = np.clip(servo_x_output, -20, 20)  # 限制单次输出变化幅度
        ...
        # 限制舵机输出变化幅度，防止过摇
        servo_y_output = np.clip(servo_y_output, -20, 20)  # 限制单次输出变化幅度
    ...
    def get_adaptive_color_range(self, target_lab, threshold):
        """获取自适应颜色范围"""
        # 根据当前光照条件动态调整颜色范围，使用更保守的参数
        base_range = 30 * threshold  # 减小基础范围，提高精度
    ...
    def get_preset_color_range(self, color_name, threshold):
        ...
        # 根据阈值动态调整范围，使用更保守的参数
        range_adjustment = 10 * threshold  # 减小调整范围，提高精度
